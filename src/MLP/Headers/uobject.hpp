...
namespace protocol::engine::sdk {
    ...
    public:
        ...
        int fname_index() {
            return memory::read<int>(reinterpret_cast<uintptr_t>(this) + 0x18); // FName index offset
        }

        u_object* super() {
            return memory::read<u_object*>(reinterpret_cast<uintptr_t>(this) + 0x40); // Super offset
        }

        u_object* class_private() {
            return memory::read<u_object*>(reinterpret_cast<uintptr_t>(this) + 0x10); // ClassPrivate offset
        }

        u_object* outer() {
            return memory::read<u_object*>(reinterpret_cast<uintptr_t>(this) + 0x20); // Outer offset
        }
