...
public:
	...
	inline double distance(ImVec2 vector_to)
	{
		return sqrt(pow(x - vector_to.x, 2) + pow(y - vector_to.y, 2));
	}
	//vector2 normalize() {
...
public:
	...
	inline vector3 crossProduct(vector3 vector_to) {
		return vector3{
			y * vector_to.z - z * vector_to.y,
			z * vector_to.x - x * vector_to.z,
			x * vector_to.y - y * vector_to.x
		};
	}
